...
template <std::size_t bufferSize> void CopyHelper<bufferSize>::copy(NativeFileStream &input, NativeFileStream &output, std::size_t count)
{
    ...
    if (output.fileDescriptor() != -1 && input.fileDescriptor() != -1) {
        ...
        output.seekp(static_cast<NativeFileStream::off_type>(totalBytes), std::ios_base::cur);
        input.seekg(static_cast<NativeFileStream::off_type>(totalBytes), std::ios_base::cur);
        return;
    }
#endif
